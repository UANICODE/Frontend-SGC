...
export function StoreBadge({ variant, disabled }: StoreBadgeProps) {
  ...
  const content = (
    <>
      <div
        className={`flex h-8 w-8 items-center justify-center rounded-2xl overflow-hidden ${
          disabled ? "bg-white/10" : "bg-white/20 shadow-md"
        }`}
      >
        {isPlay ? (
          // Mantém o ícone no mesmo lugar
  ...
  return (
    <MotionLink
      href={isPlay ? "https://play.google.com/store/apps/details?id=com.foodnect.app" : "#"}
      target="_blank"
