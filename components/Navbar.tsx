...
import { Home, UtensilsCrossed, Phone, LogIn, Download, MapPin, Star, Menu, X } from "lucide-react";
    ...
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:py-4 md:px-6">
        ...
        <div className="hidden items-center gap-2 text-sm font-medium md:flex">
          {navItems.map((item, idx) => {
            const isActive = item.href.includes("#") 
              ? pathname === item.href.split("#")[0] || (pathname === "/" && item.href.startsWith("/#"))
              : pathname === item.href;
                ...
                <Link
                  ...
                  className={`relative flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-200 group ${
                    isActive
                      ...
                      : "text-textPrimaryLight hover:text-primary"
                  }`}
                >
        ...
        <div className="flex items-center gap-2">
          {/* CTA principal - apenas desktop para não ocupar o cabeçalho no mobile */}
          <motion.a
            className="hidden md:flex items-center gap-2 rounded-full bg-gradient-to-r from-primary via-[#FF9E3A] to-primary bg-size-200 bg-pos-0 hover:bg-pos-100 px-6 py-2.5 text-sm font-bold text-white shadow-xl shadow-primary/50 transition-all duration-500 shine-effect animate-pulse-glow"
            whileHover={{ scale: 1.07, boxShadow: "0 12px 34px rgba(255, 122, 26, 0.5)" }}
            whileTap={{ scale: 0.95 }}
            href="#download"
            onClick={(e) => {
              e.preventDefault();
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={handleClick}
                  className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-textSecondaryLight hover:text-textPrimaryLight hover:bg-backgroundLight"
                >
